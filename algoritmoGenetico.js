...
fobjetivo = function(x) {
  var f = Math.pow(x, 4) + 5 * Math.pow(x, 3) + 4 * Math.pow(x, 2) - (4 * x) +
    1;
  return (-1 * f) + 20;
}
...
ReproduceInd = function(Padre1, Padre2) {
    ...
    hijos = {
      h1: new Individuo(h1genes),
      h2: new Individuo(h2genes)
    };
    return hijos;
  }
  ///////////////////////////////////////////////////////////////
...
while (generaciones++ <= generacionesParo) {
  ...
  for (var j in Poblacion) {
    Poblacion[j].prob = parseFloat(Poblacion[j].getFitness / fitnessTotal) *
      100.0;
